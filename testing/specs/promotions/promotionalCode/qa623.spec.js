...
describe('QA-623 No es posible aplicar una promoción con un código promocional que tenga como fecha fin una fecha inferior a la actual', function() {
    beforeAll(function() {
    ...
    it('Avanzamos hasta la pantalla de validar carrito, desde el carrito, insertamos un código que no esté activo por fecha, y validamos que no se valida la promoción', function(){
        ...
        PromotionsPO.setPromotionalCode(genericData.promotions.promotionalCodeLastDate).then(function(promotionalCodeInputTxt){
            expect(promotionalCodeInputTxt).toEqual(genericData.promotions.promotionalCodeLastDate);
