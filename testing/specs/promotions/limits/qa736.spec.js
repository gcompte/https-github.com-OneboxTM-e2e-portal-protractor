...
var env = browser.params.env,
    ...
    qaDataActTest = qaData.qa736;

describe('QA-736 Actualización de contador de usos de 3 promociones distintas con Limites máximos al liberar localidades por navegar a otra sesión', function() {
    beforeAll(function() {
    ...
    it('Verificamos que los contadores de las promociones se han actualizado correctamente', function() {
        ...
        protractor.promise.controlFlow().wait(PromotionsPO.getCounterValue(qaDataGeneric.eventId, qaDataActTest.promotionDescId, 'event')).then(function (result) {
            expect(result.counter).toEqual(qaDataActTest.seatsToSelect);
        });
        protractor.promise.controlFlow().wait(PromotionsPO.getCounterValue(qaDataGeneric.eventId, qaDataActTest.promotionPromoId, 'event')).then(function (result) {
            expect(result.counter).toEqual(qaDataActTest.seatsToSelect);
        });
    });
